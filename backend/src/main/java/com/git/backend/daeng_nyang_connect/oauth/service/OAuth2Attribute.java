...
public class OAuth2Attribute {
    ...
    /**
     * 구글 로그인일 경우 사용하는 메서드, 사용자 정보가 따로 wrapping 되지 않고 제공되어,
     * 바로 get() 메서드로 접근이 가능!
     */
    ...
    /**
      kakao 로그인일 경우 사용하는 메서드, 필요한 사용자 정보가 kakaoAccount -> kakaoProfile 두번 감싸져 있어
      두번 get() 메서드를 이용해 사용자 정보를 담고 있는 Map 을 꺼내야 한다
     */
    ...
    private static OAuth2Attribute ofKakao(String provider, String attributeKey,
        ...
        return OAuth2Attribute.builder()
                .email((String) kakaoProfile.get("profile_nickname"))
    ...
    /**
     * Naver 로그인일 경우 사용하는 메서드, 필요한 사용자 정보가 response Map 에 감싸져 있어서,
     * 한번 get() 메서드를 이용해 사용자 정보를 담고있는 Map 을 꺼내야한다.
     *
     */
